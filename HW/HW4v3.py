...
def get_info_from_vectorDB(collection, question, n_results=N_RESULTS, max_orgs=MAX_ORGS):
    ...
    if labels:
        context = (f'FILTER NOTE: {matched} organizations in the directory match '
                   f'"{description}". The {len(order)} records below are a sample of them, '
                   f'not the complete list. State the total when it is useful.\n\n') + context
