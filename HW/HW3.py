...
st.write(
    "This chatbot answers questions about web pages that you choose. Add one or two "
    "URLs in the sidebar and click Load URLs. The text of those pages is pulled down, "
    "stripped of blank lines, and placed in a system prompt that is rebuilt and re-sent "
    "on every request, so your source material is never dropped from context no matter "
    "how long the conversation runs. Answers are drawn only from those pages, and each "
    "answer names the document it came from. If the pages do not cover what you asked, "
    "the bot says so instead of guessing.\n\n"
    "Pick GPT-6 Astra or Claude Fable 5.1 in the sidebar, and switch between them at any "
    "point in a conversation.\n\n"
    f"Conversation memory is a rolling {MEMORY_TOKEN_BUDGET:,}-token buffer. Before each "
    "request the history is walked backwards from your newest message, and older turns "
    "are left out once adding the next one would push the total past the limit. Only that "
    "buffered slice is sent to the model, while the full transcript stays on screen. The "
    "loaded documents are budgeted separately and never count against those "
    f"{MEMORY_TOKEN_BUDGET:,} tokens."
)
...
st.sidebar.caption(f"_Using {MODELS[provider]['label']} ({model_to_use})_")

st.sidebar.subheader("Memory")
st.sidebar.caption(
    f"Rolling buffer of {MEMORY_TOKEN_BUDGET:,} tokens of conversation. "
    "Source documents sit outside this budget and are never dropped."
)
